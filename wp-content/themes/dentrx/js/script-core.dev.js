(function($){
	...
	$.fn.realfactory_mobile_menu = function( args ){
		...
		$(this).find('a').not('.mm-next').click(function(){
			menu_api.close();
		});
	...
	var realfactory_anchor = function(){
		...
		this.anchor_link = $('a[href^="#"]').not('[href="#"]').filter(function(){
			...
			// for mm-menu plugin
			if( $(this).is('.realfactory-mm-menu-button, .mm-next, .mm-prev, .mm-title') ){
				return false;
			}

			// for woocommerce
			if( $(this).parent('.description_tab, .reviews_tab').length ){
				return false;
			}
