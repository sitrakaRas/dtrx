 ...
 */
	...
	while( have_posts() ){ the_post();
		...
		if( $sidebar_type == 'none' ){
			...
			echo '<div id="map-canvas"></div>'; // custom map with filter
...
<script type="text/javascript">
	jQuery(document).ready(function($) {
		...
		$('.filterCat').click(function() {			
			var curFilter = $(this).parent('.gdlr-core-column-15').index() - 1;
			$('.wpgmp_toggle_main_container .wpgmp_tab_item input').attr('checked', false).trigger('click');
			$('.wpgmp_toggle_main_container .wpgmp_tab_item').eq(curFilter).find('input').attr('checked', true).trigger('click');
		});
	});
</script>
