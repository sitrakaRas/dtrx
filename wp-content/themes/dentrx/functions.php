	...
	*/
	...
	function configuration_post_type() {
		register_taxonomy(
			...
			'post',
			array(
				'label' => __( 'Categories Partenaire' ),
				'rewrite' => array( 'slug' => 'part-cat' ),
				'hierarchical' => true
			)
		);
		...
		$args = array(
			...
			'taxonomies'          => array( 'partenaire-category' ),
		);
	...
	//sidebar shortcode
	function sidebar_partenaire() {
		ob_start();
		get_template_part('content/partenaire-sidebar');
		return ob_get_clean();   
	} 
	add_shortcode( 'sidebar_partenaire', 'sidebar_partenaire' );

	//listing partenaire shortcode
	function listing_partenaire() {
		ob_start();
		get_template_part('content/partenaire-listing');
		return ob_get_clean();   
	} 
